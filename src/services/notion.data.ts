...
import { MasterId, Entry, RowData, Blog, Post } from '../utils/types';

export async function retrieveNotionData(entry: Entry){
    const masterId = transformStringIntoId(entry);

    function transformStringIntoId(str: Entry) {
    ...
    async function requestData(id : MasterId): Promise<RowData | undefined> {
        const options = {
            headers: {
